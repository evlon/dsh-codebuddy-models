...
模型目录**自动从企业内置模型接口获取**（`www.codebuddy.cn/console/enterprises/{enterpriseId}/builtin-models`，用桌面端登录态鉴权，10 分钟缓存）：企业账号能看到并启用的模型会自动出现在选择器里，无需维护硬编码列表。设置页的「CodeBuddy 模型」区块会**只读展示**这份自动获取的目录。
